...
for PENALTY2 in $PENALTIES; do
    ...
    if [ ! -f $OUT ]; then
        echo $OUT
        python3 train.py $DATASET GaussianGrid --epochs $EPOCHS --penalty1 $PENALTY1 --penalty2 $PENALTY2 &
    fi
    OUT="model-$DATASET-GaussianGrid-penalty-$PENALTY1-$PENALTY2-usesoftmax-1.pth"
    if [ ! -f $OUT ]; then
        echo $OUT
        python3 train.py $DATASET GaussianGrid --epochs $EPOCHS --penalty1 $PENALTY1 --penalty2 $PENALTY2 --use-softmax &
    fi
    wait
done
done
