...
class PrototypeLayer(torch.nn.Module):
    ...
    def forward(self, z):
        ...
        p = torch.flatten(self.prototypes, 1, 2)
        ...
        heatmap = torch.log((distances+1)/(distances+1e-7))  # (B, P, H, W)
        ...
        # input image.
        similarity = heatmap.amax((2, 3))  # (B, P)
        return heatmap, similarity
...
class ProtoPNet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        heatmap, similarity = self.prototype_layer(features)
        ...
        return {'class': scores, 'features': features, 'prototypes_heatmap': heatmap}
